...
-- Este rollback usa public.training_session_consolidation_audit.rollback_payload.
-- Este archivo NO es una migracion y no debe moverse a supabase/migrations.

begin;

-- Prevalidaciones de rollback.
do $$
declare
...
begin
  ...
  if v_executed_count <> 5 then
    raise exception 'Rollback QA esperado: 5 auditorias executed sin rollback; encontrado %', v_executed_count;
  end if;
  ...
  if v_rolled_back_count <> 0 then
    raise exception 'Rollback abortado: existen auditorias ya revertidas %', v_rolled_back_count;
  end if;
  ...
  if v_missing_payload_count <> 0 then
    raise exception 'Rollback abortado: rollback_payload invalido en % auditorias', v_missing_payload_count;
  end if;
end $$;

create temp table qa_rollback_audit on commit drop as
select *
from public.training_session_consolidation_audit
where status = 'executed'
  and executed_at is not null
...
-- Restaurar exercise_entries.session_id original por entry id.
update public.exercise_entries e
set session_id = r.original_session_id
from qa_rollback_entries r
where e.id = r.entry_id;

-- Restaurar campos originales de sesion canonica y no canonicas.
...
from qa_rollback_sessions r
where s.id = r.session_id;

-- Marcar auditoria como revertida.
update public.training_session_consolidation_audit a
set
  status = 'rolled_back',
  rolled_back_at = now()
from qa_rollback_audit r
where a.id = r.id
...
begin
  ...
  if v_not_rolled_back_count <> 0 then
    raise exception 'Rollback fallo: auditorias no marcadas como rolled_back %', v_not_rolled_back_count;
  end if;
  ...
  if v_entry_mismatch_count <> 0 then
    raise exception 'Rollback fallo: entries con session_id no restaurado %', v_entry_mismatch_count;
  end if;
  ...
  if v_session_mismatch_count <> 0 then
    raise exception 'Rollback fallo: campos de sesion no restaurados %', v_session_mismatch_count;
  end if;
  ...
  if v_active_audit_count <> 0 then
    raise exception 'Rollback fallo: quedan auditorias executed sin rollback %', v_active_audit_count;
  end if;
end $$;
