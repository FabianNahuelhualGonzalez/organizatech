...
-- Requiere auditoria Claude antes de cualquier ejecucion.
-- Requiere aprobacion humana antes de ejecucion.
-- Este archivo NO es una migracion y no debe moverse a supabase/migrations.
-- Precheck manual obligatorio antes de ejecutar:
-- select count(*)
-- from public.exercise_entries e
-- join public.training_sessions s on s.id = e.session_id
-- where s.deleted_at is not null;
-- Debe retornar 0. Si retorna mayor a 0, no ejecutar la consolidacion.
-- Nota completed_at QA:
-- completed_at = coalesce(s.completed_at, s.trained_at, s.created_at)
-- queda permitido solo para QA. Produccion requiere aprobacion de Arquitectura
-- por posible conversion implicita date -> timestamptz segun zona horaria SQL.

begin;

-- A) Crear/asegurar tabla administrativa de auditoria.
-- Estrategia de acceso:
-- - Tabla administrativa.
-- - Sin acceso frontend.
-- - RLS habilitado sin policies para usuarios normales.
-- - GRANT controlado solo para roles operativos aprobados fuera de este borrador.
...
-- B) Construir grupos legacy validos.
create temp table qa_legacy_group_details on commit drop as
select
  s.user_id,
  s.trained_at,
  s.week_number,
...
create temp table qa_consolidation_candidates on commit drop as
select
  d.*,
  ...
  d.trained_at::date as inferred_trained_date,
  (
    d.trained_at::date - (extract(isodow from d.trained_at::date)::integer - 1)
  )::date as inferred_calendar_week_start
from qa_legacy_group_details d
...
alter table qa_consolidation_candidates
  add column inferred_planned_date date;

update qa_consolidation_candidates
...
-- C) Prevalidar conteos esperados de QA controlado antes de cualquier cambio de datos.
do $$
declare
...
begin
  select count(*) into v_group_count from qa_consolidation_candidates;
  if v_group_count <> 5 then
    raise exception 'QA esperado: 5 grupos consolidables; encontrado %', v_group_count;
  end if;

  select coalesce(sum(legacy_session_count), 0) into v_session_count from qa_consolidation_candidates;
  if v_session_count <> 30 then
    raise exception 'QA esperado: 30 training_sessions legacy; encontrado %', v_session_count;
  end if;

  select coalesce(sum(entry_count), 0) into v_entry_count from qa_consolidation_candidates;
  if v_entry_count <> 30 then
    raise exception 'QA esperado: 30 exercise_entries legacy; encontrado %', v_entry_count;
  end if;

  select count(distinct user_id) into v_user_count from qa_consolidation_candidates;
  if v_user_count <> 2 then
    raise exception 'QA esperado: 2 usuarios afectados; encontrado %', v_user_count;
  end if;

  select coalesce(sum(orphan_entry_count), 0) into v_orphan_count from qa_legacy_group_details;
  if v_orphan_count <> 0 then
    raise exception 'QA esperado: 0 entries huerfanas; encontrado %', v_orphan_count;
  end if;

  select count(*) into v_mixed_routine_count
  from qa_legacy_group_details
  where routine_count <> 1;
  if v_mixed_routine_count <> 0 then
    raise exception 'QA esperado: 0 grupos con mezcla de rutinas; encontrado %', v_mixed_routine_count;
  end if;
  ...
  if v_mixed_day_count <> 0 then
    raise exception 'QA esperado: 0 grupos con mezcla de dias; encontrado %', v_mixed_day_count;
  end if;

  select coalesce(sum(ownership_issue_count), 0) into v_ownership_issue_count from qa_legacy_group_details;
  if v_ownership_issue_count <> 0 then
    raise exception 'QA esperado: 0 ownership issues; encontrado %', v_ownership_issue_count;
  end if;
  ...
  where not exists (
    select 1
    from qa_consolidation_candidates c
    where c.user_id = d.user_id
      and c.trained_at = d.trained_at
      and c.week_number = d.week_number
  );
  if v_non_candidate_count <> 0 then
    raise exception 'QA esperado: todos los grupos consolidation_candidate; grupos no candidatos %', v_non_candidate_count;
  end if;

  select count(*) into v_bad_canonical_count
  from qa_consolidation_candidates
  where canonical_session_id is distinct from legacy_session_ids[1];
  if v_bad_canonical_count <> 0 then
    raise exception 'QA esperado: canonical_session_id = legacy_session_ids[1]; inconsistencias %', v_bad_canonical_count;
  end if;
  ...
  if v_bad_planned_day_count <> 0 then
    raise exception 'QA esperado: planned_day tecnico y planned_date inferibles; inconsistencias %', v_bad_planned_day_count;
  end if;
  ...
  if v_existing_audit_count <> 0 then
    raise exception 'Existen registros de auditoria pendientes/ejecutados sin rollback: %', v_existing_audit_count;
  end if;
  ...
  if v_entries_soft_deleted_count <> 0 then
    raise exception 'Precheck fallo: entries apuntando a sesiones soft-deleted %', v_entries_soft_deleted_count;
  end if;
end $$;

create temp table qa_metrics_before on commit drop as
select
  s.user_id,
...
-- D) Insertar auditoria con rollback_payload formal antes de modificar datos.
...
select
  c.user_id,
  concat(c.user_id::text, ':', c.trained_at::text, ':', c.week_number::text) as legacy_group_key,
  ...
  jsonb_build_object(
    ...
    'non_canonical_sessions', (
      ...
      from public.training_sessions s
      where s.id = any(array_remove(c.legacy_session_ids, c.canonical_session_id))
    )
  )
from qa_consolidation_candidates c;
...
begin
  ...
  if v_audit_count <> 5 then
    raise exception 'QA esperado: 5 registros de auditoria pending; encontrado %', v_audit_count;
  end if;
end $$;

-- E) Actualizar sesion canonica.
...
set
  ...
  status = 'completed',
  -- Permitido solo para QA. Produccion requiere aprobacion de Arquitectura
  -- por posible conversion implicita date -> timestamptz segun zona horaria SQL.
  completed_at = coalesce(s.completed_at, s.trained_at, s.created_at),
  deleted_at = null
from qa_consolidation_candidates c
...
-- F) Mover exercise_entries hacia la sesion canonica.
update public.exercise_entries e
set session_id = c.canonical_session_id
from qa_consolidation_candidates c
where e.id = any(c.entry_ids);

-- G) Marcar sesiones no canonicas como eliminadas logicamente.
update public.training_sessions s
set deleted_at = now()
from qa_consolidation_candidates c
where s.id = any(array_remove(c.legacy_session_ids, c.canonical_session_id));

-- H) Marcar auditoria como ejecutada.
update public.training_session_consolidation_audit a
set
  status = 'executed',
  executed_at = now()
from qa_consolidation_candidates c
where a.legacy_group_key = concat(c.user_id::text, ':', c.trained_at::text, ':', c.week_number::text)
  and a.status = 'pending'
  and a.rolled_back_at is null;

-- I) Validaciones post-consolidacion.
create temp table qa_metrics_after on commit drop as
select
  s.user_id,
  count(distinct s.id) as session_count,
  count(e.id) as entry_count,
...
do $$
declare
  v_bad_metrics_count integer;
  v_entries_without_session_count integer;
  v_entries_soft_deleted_count integer;
  v_active_duplicate_count integer;
  v_group_without_one_active_count integer;
  v_ownership_issue_count integer;
  v_audit_executed_count integer;
  v_entry_count integer;
begin
  select count(*) into v_bad_metrics_count
  from qa_metrics_before b
  full join qa_metrics_after a using (user_id)
  where b.entry_count is distinct from a.entry_count
     or b.total_reps is distinct from a.total_reps
     or b.simple_weight_total is distinct from a.simple_weight_total
     or b.volume_total is distinct from a.volume_total;
  if v_bad_metrics_count <> 0 then
    raise exception 'Validacion post fallo: totales por usuario cambiaron en % usuarios', v_bad_metrics_count;
  end if;

  select count(*) into v_entries_without_session_count
  from public.exercise_entries
  where session_id is null;
  if v_entries_without_session_count <> 0 then
    raise exception 'Validacion post fallo: entries sin session_id %', v_entries_without_session_count;
  end if;
  ...
  where s.deleted_at is not null;
  if v_entries_soft_deleted_count <> 0 then
    raise exception 'Validacion post fallo: entries apuntando a sesiones soft-deleted %', v_entries_soft_deleted_count;
  end if;

  select count(*) into v_active_duplicate_count
  from (
    select user_id, routine_id, trained_date
  ...
  if v_active_duplicate_count <> 0 then
    raise exception 'Validacion post fallo: sesiones activas duplicadas %', v_active_duplicate_count;
  end if;

  select count(*) into v_group_without_one_active_count
  from qa_consolidation_candidates c
  where (
    select count(*)
    from public.training_sessions s
  ...
  if v_group_without_one_active_count <> 0 then
    raise exception 'Validacion post fallo: grupos sin exactamente 1 sesion activa %', v_group_without_one_active_count;
  end if;
  ...
  where e.user_id <> s.user_id
     or ex.user_id <> s.user_id;
  if v_ownership_issue_count <> 0 then
    raise exception 'Validacion post fallo: mezcla de usuarios %', v_ownership_issue_count;
  end if;

  select count(*) into v_audit_executed_count
  from public.training_session_consolidation_audit
  where status = 'executed'
    and executed_at is not null
    and rolled_back_at is null;
  if v_audit_executed_count <> 5 then
    raise exception 'Validacion post fallo: auditoria executed esperada 5; encontrado %', v_audit_executed_count;
  end if;
  ...
  if v_entry_count <> 30 then
    raise exception 'Validacion post fallo: 30 entries legacy esperadas; encontrado %', v_entry_count;
  end if;
end $$;
