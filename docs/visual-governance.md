...
- Todas las notificaciones actuales y futuras se entregan exclusivamente mediante la campanita y su centro de notificaciones. Ningún Dashboard, Perfil o feature puede mostrar una tarjeta permanente que represente o duplique una notificación. Los accesos del menú son navegación, no notificaciones.
- Al volver a Safari desde foreground, una sesión Usuario ya autorizada que revalida la misma identidad conserva en memoria el shell y la pantalla actuales; no monta `AuthLoadingScreen` ni reinicia la navegación. Esta excepción sólo cubre la capacidad efímera vigente A→A: no persiste ni restaura formularios, correos, tokens, códigos o datos sensibles. No cubre una recarga fría ni el descarte del proceso o pestaña por Safari. Logout, sesión ausente o vencida, A→B, recovery, autorización inválida y errores no recuperables desmontan el shell previo y fallan cerrados. Login, registro, recovery y arranque frío sin autorización mantienen la pantalla de carga existente.
