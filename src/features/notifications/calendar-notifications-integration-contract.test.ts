...
test("Usuario y Coach usan scopes persistidos separados y Coach no recibe catálogo Usuario", () => {
  ...
  assert.match(root, /\.\.\.persistedCalendarNotifications\.notifications,[\s\S]*\.\.\.persistedTrainingCycleNotifications\.notifications,[\s\S]*\.\.\.persistedCoachLinkNotifications\.notifications/);
  assert.match(root, /\.\.\.persistedCalendarNotifications\.seenRecords,[\s\S]*\.\.\.persistedTrainingCycleNotifications\.seenRecords,[\s\S]*\.\.\.persistedCoachLinkNotifications\.seenRecords/);
  assert.match(root, /additionalNotifications: persistedNotifications/);
  assert.match(root, /persistedSeenRecords,/);
...
test("read_at remoto es autoridad de calendar y localStorage no puede ocultar rollback", () => {
  ...
  assert.match(root, /if \(!isNotificationPanelOpen\) \{[\s\S]*persistedCalendarNotifications\.reload\(\)[\s\S]*persistedTrainingCycleNotifications\.reload\(\)[\s\S]*persistedCoachLinkNotifications\.reload\(\)/);
});
