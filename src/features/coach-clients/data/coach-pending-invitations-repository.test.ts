...
import type { CapturedCoachPendingInvitationsOperation, CoachPendingInvitationsParameters, CoachPendingInvitationsQuery } from "./coach-pending-invitations-contract";
import { createCoachPendingInvitationsRepository } from "./coach-pending-invitations-repository";

const owner = "10000000-0000-4000-8000-000000000001";
const query: CoachPendingInvitationsQuery = { query: "", limit: 25, cursor: null };
const empty = () => ({ serverNow: "2026-09-09T09:00:00Z", totalPending: 0, matchingCount: 0, items: [], nextCursor: null });
...
function fixture(timeoutMilliseconds?: number) {
  let current = true;
  let captures = 0;
  const calls: { name: string; args: CoachPendingInvitationsParameters; signal: AbortSignal }[] = [];
  let transport = async (): Promise<{ data: unknown; error: unknown }> => ({ data: empty(), error: null });
  const operation: CapturedCoachPendingInvitationsOperation = {
  ...
  const repo = createCoachPendingInvitationsRepository({ timeoutMilliseconds,
...
test("list emits only exact read RPC parameters and does not cache or retry", async () => {
  const f = fixture();
  assert.deepEqual(await f.repo.listPendingInvitations(query), empty());
  assert.equal(f.calls[0].name, "list_own_pending_coach_invitations");
  assert.deepEqual(f.calls[0].args, { p_query: "", p_limit: 25, p_cursor_created_at: null, p_cursor_id: null });
  assert.ok(Object.isFrozen(f.calls[0].args));
  await f.repo.listPendingInvitations(query);
...
test("invalid raw inputs and already-aborted requests never capture", async () => {
  const f = fixture();
  await assert.rejects(f.repo.listPendingInvitations({ ...query, user_id: owner } as CoachPendingInvitationsQuery), issue("invalid_input"));
  const stop = new AbortController(); stop.abort();
  await assert.rejects(f.repo.listPendingInvitations(query, { signal: stop.signal }), issue("aborted"));
  assert.equal(f.captures(), 0);
});

test("snapshot query and cursor before asynchronous capture", async () => {
  const f = fixture(); const capture = deferred<CapturedCoachPendingInvitationsOperation>();
  f.capture(() => capture.promise);
  const input = { query: "alumno", limit: 25, cursor: { id: owner, createdAt: "2026-09-09T09:00:00Z" } };
  const promise = f.repo.listPendingInvitations(input);
...
test("capture timeout and abort prevent late RPC dispatch", async () => {
  for (const abort of [false, true]) {
    const f = fixture(15), waiting = deferred<CapturedCoachPendingInvitationsOperation>();
    f.capture(() => waiting.promise); const stop = new AbortController();
    const promise = f.repo.listPendingInvitations(query, { signal: stop.signal });
    if (abort) stop.abort();
...
test("transport timeout aborts signal even if source ignores it, late result cannot escape", async () => {
  ...
  await assert.rejects(f.repo.listPendingInvitations(query), issue("timeout"));
...
test("explicit abort after dispatch rejects and leaves no second call", async () => {
  ...
  const promise = f.repo.listPendingInvitations(query, { signal: stop.signal }); await tick(); stop.abort();
...
test("identity change rejects before RPC and after resolved/rejected transport", async () => {
  const before = fixture(); before.stale(); await assert.rejects(before.repo.listPendingInvitations(query), issue("operation_stale"));
  ...
  for (const reject of [false, true]) {
    ...
    await assert.rejects(f.repo.listPendingInvitations(query), issue("operation_stale"));
  }
});
...
test("SQL errors sanitize known codes and never project errors as empty portfolio", async () => {
  for (const [code, expected] of [["42501", "forbidden"], ["22023", "invalid_input"], ["other", "unavailable"]] as const) {
    ...
    await assert.rejects(f.repo.listPendingInvitations(query), issue(expected)); assert.equal(f.calls.length, 1);
  }
});
...
test("invalid JSON/data and accessors fail closed, no accessor is executed", async () => {
  ...
  for (const result of [{ data: null, error: null }, { data: {}, error: null }, bad]) {
    ...
    await assert.rejects(f.repo.listPendingInvitations(query), issue("invalid_response"));
  }
...
test("hostile capture/transport errors and malformed identity are sanitized", async () => {
  ...
  for (const thrown of [Error("private"), revoked.proxy, { get code() { throw Error("private"); } }]) {
    ...
    await assert.rejects(f.repo.listPendingInvitations(query), issue("unavailable"));
  }
  const f = fixture(); f.capture(async () => ({ ...f.operation, identity: { userId: owner, generation: -1 } }));
  await assert.rejects(f.repo.listPendingInvitations(query), issue("invalid_response")); assert.equal(f.calls.length, 0);
});
