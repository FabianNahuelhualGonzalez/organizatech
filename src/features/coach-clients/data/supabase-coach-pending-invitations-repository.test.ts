...
import { createSupabaseCoachPendingInvitationsRepository } from "./supabase-coach-pending-invitations-repository";
...
const empty = () => ({ serverNow: "2026-09-09T09:00:00.123456Z", totalPending: 0, matchingCount: 0, items: [], nextCursor: null });
...
function fixture(timeoutMilliseconds?: number) {
  ...
  const repo = createSupabaseCoachPendingInvitationsRepository({ configuration: config, expectedIdentity, timeoutMilliseconds,
...
test("real SDK POST serializes limit and null cursors with pinned principal, no Auth fetch", async () => {
  const f = fixture(); assert.deepEqual(await f.repo.listPendingInvitations(query), empty());
  assert.equal(f.requests.length, 1); assert.deepEqual(f.counts(), { sessionCalls: 1, userCalls: 1 });
  assert.deepEqual(JSON.parse(String(f.requests[0].body)), { p_query: "", p_limit: 25, p_cursor_created_at: null, p_cursor_id: null });
});

test("SDK sends exact original bounded search and preserves microsecond cursor", async () => {
  const f = fixture(); await f.repo.listPendingInvitations({ query: " ÁLUMNO% ", limit: 50,
    cursor: { id: owner, createdAt: "2026-09-09T09:00:00.123456Z" } });
  assert.deepEqual(JSON.parse(String(f.requests[0].body)), { p_query: " ÁLUMNO% ", p_limit: 50,
    p_cursor_created_at: "2026-09-09T09:00:00.123456Z", p_cursor_id: owner });
});
...
test("public configuration and expected identity are snapshotted once", async () => {
  ...
  await f.repo.listPendingInvitations(query); assert.equal(f.requests.length, 1);
});

test("invalid input never reads Auth or dispatches SDK fetch", async () => {
  const f = fixture(); await assert.rejects(f.repo.listPendingInvitations({ ...query, limit: 100 }), issue("invalid_input"));
...
test("verified principal mismatch is forbidden without network dispatch", async () => {
  ...
  await assert.rejects(f.repo.listPendingInvitations(query), issue("forbidden")); assert.equal(f.requests.length, 0);
});
...
test("hung Auth consumes same deadline and cannot later initiate verified-user or RPC calls", async () => {
  ...
  f.session(() => waiting.promise); await assert.rejects(f.repo.listPendingInvitations(query), issue("timeout"));
...
test("SDK malformed response/HTTP error fails, no stale or fabricated empty page", async () => {
  for (const [payload, status, expected] of [[{ code: "42501", message: "private" }, 403, "forbidden"],
    ...
    await assert.rejects(f.repo.listPendingInvitations(query), issue(expected)); assert.equal(f.requests.length, 1);
  }
});

test("SDK disables transient auto-retry on 503", async () => {
  const f = fixture(); f.transport(async () => json({ message: "private" }, 503));
  await assert.rejects(f.repo.listPendingInvitations(query), issue("unavailable")); assert.equal(f.requests.length, 1);
});
...
test("caller abort propagates to real SDK fetch signal and ignores late response", async () => {
  ...
  const stop = new AbortController(), promise = f.repo.listPendingInvitations(query, { signal: stop.signal });
