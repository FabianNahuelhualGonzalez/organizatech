...
assert.match(appSource, /setSetupDay\(result\.state\.activeDay\)/);
...
assert.match(appSource, /async function saveInitialRoutine\(confirmedRoutineUpdate = false\)/);
assert.match(appSource, /saveRoutine=\{saveInitialRoutine\}/);
assert.match(appSource, /onClick=\{saveRoutine\}/);
assert.match(appSource, /routineUpdateConfirmed: Boolean\(confirmedRoutineUpdate\)/);
...
assert.match(appSource, /setSetupDay\(setupTransition\.nextDay\)/);
...
assert.equal(testCommands.length, 117);
